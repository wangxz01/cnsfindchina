...
迁移不带的（已在 `.gitignore`）：
- `browser_profile*/`（含 Cookie，新设备会重新过验证）
- `cache*/`（已抓缓存）
- 输出 `*.xlsx`
...
```bash
python unified_web.py            # 默认 http://127.0.0.1:8000/
python unified_web.py --port 8080
```
...
```bash
python scraper.py            # Cell    → cell_YYYY-MM-DD.xlsx
python nature_scraper.py     # Nature  → nature_YYYY-MM-DD.xlsx
python science_scraper.py    # Science → science_YYYY-MM-DD.xlsx
```
...
| Source | 文件 | URL 模式 |
|---|---|---|
| Cell | `urls.txt` | `sciencedirect.com/journal/cell/vol/<v>/issue/<i>` |
| Nature | `urls_nature.txt` | `nature.com/nature/volumes/<v>/issues/<i>` |
| Science | `urls_science.txt` | `science.org/toc/science/<v>/<i>` |
...
| 文件 | 作用 |
|---|---|
| `unified_web.py` | **统一 Web 服务（推荐入口）** |
| `static/unified_index.html` | 前端单页（Vue 3 CDN，无构建） |
| `scraper.py` | Cell 抓取逻辑（CLI 入口 + 三套共用的 ScraperCallbacks / CF 检测 / 随机化辅助） |
| `nature_scraper.py` | Nature 抓取逻辑 + 国别判定函数（`parse_country`） |
| `science_scraper.py` | Science 抓取逻辑 |
| `excel_writer.py` | Excel 输出（多 sheet + 自定义列 schema） |
| `urls.txt` / `urls_nature.txt` / `urls_science.txt` | 各 source 的 issue URL 列表 |
| `requirements.txt` | 依赖 |
| `browser_profile*/` | Playwright 持久化浏览器配置（运行后生成，不入库） |
| `cache*/` | 按 PII/DOI 的文章缓存（运行后生成，不入库） |
