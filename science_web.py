"""Science 期刊爬虫 Web 服务（镜像 nature_web.py，调用 science_scraper）。

启动：
    python science_web.py [--host 127.0.0.1] [--port 8002]
"""
...
app = FastAPI(title="Science Scraper")
...
def index():
    idx = STATIC_DIR / "science_index.html"
    if not idx.exists():
        fallback = STATIC_DIR / "nature_index.html"
        if fallback.exists():
            return FileResponse(str(fallback))
        return HTMLResponse("<h1>static/science_index.html 不存在</h1>", status_code=500)
...
def favicon():
    svg = (
        ...
        '<rect width="64" height="64" rx="10" fill="#1a4480"/>'
        '<text x="32" y="44" font-size="36" text-anchor="middle" '
        'fill="#fff" font-family="serif" font-weight="bold">S</text>'
        '</svg>'
    ).encode("utf-8")
...
def main():
    ap = argparse.ArgumentParser(description="Science scraper web UI")
    ap.add_argument("--host", default="127.0.0.1")
    ap.add_argument("--port", type=int, default=8002)
    args = ap.parse_args()

    import uvicorn
    print(f"[science-web] 启动：http://{args.host}:{args.port}/  (Ctrl+C 退出)")
