...
class CatalogTests(unittest.TestCase):
    ...
    def test_worker_error_and_cancel_preserve_completed_years(self):
        ...
        def fail(mode, years, known, profile, stop, update):
        ...
        def cancel(mode, years, known, profile, stop, update):
            stop.wait(5)
...
class CatalogWebTests(unittest.TestCase):
    def setUp(self):
        for obj, attr, value in ((web, 'CATALOG', catalog.CatalogSession()),
    ...
    def test_invalid_requests_do_not_launch_browser(self):
        ...
        with self.assertRaises(web.HTTPException): web.catalog_status('nature')
        self.assertIsNone(web.CATALOG.thread)

    def test_scraper_and_catalog_cannot_share_browser_profile(self):
        web.SESSIONS['cell'].thread = object()
        with self.assertRaises(web.HTTPException) as exc: web.catalog_start({}, 'cell')
        self.assertEqual(exc.exception.status_code, 409)
        web.SESSIONS['cell'].thread = None
        web.CATALOG.thread = object()
        with self.assertRaises(web.HTTPException) as exc:
            web.start({'urls': ['https://www.sciencedirect.com/journal/cell/vol/189/issue/1']}, 'cell')
        self.assertEqual(exc.exception.status_code, 409)

    def test_concurrent_catalog_requests_launch_once(self):
        barrier, release = threading.Barrier(2), threading.Event()
        calls = []
        def read(*args): calls.append(1); release.wait(5)
        def start():
            barrier.wait()
